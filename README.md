...
4. Файлы скинов (`.png`, 64x64 или 64x32) кладите в папку `.minecraft/config/`
   **на стороне клиента** — сервер передаёт только UUID и имя файла, сам файл
   должен физически лежать в конфиге у каждого клиента.

5. Команда в игре:

```
/settexture <ник_или_UUID> <имя_файла.png>
```

   Требует уровень прав 2 (администратор/оператор).
