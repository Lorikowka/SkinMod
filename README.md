...
4. Файлы скинов (`.png`, 64x64 или 64x32) кладите в папку

    ```
    .minecraft/versions/<версия>/skins/
    ```

    **на стороне клиента** — сервер передаёт только UUID и имя файла, сам файл
    должен физически лежать в этой папке у каждого клиента. Папка `skins`
    создаётся автоматически при первом применении скина.
